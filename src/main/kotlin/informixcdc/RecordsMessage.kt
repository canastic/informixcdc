...
sealed class RecordsMessage {

    class Heartbeat() : RecordsMessage() {
    ...
    companion object {
        fun converter(klaxon: Klaxon) = object : Converter {
            ...
            override fun canConvert(cls: Class<*>): Boolean =
                enabled && RecordsMessage::class.java.isAssignableFrom(cls)

            override fun fromJson(jv: JsonValue): Any? = jv.obj!!["type"].let { v ->
                try { enabled = false; when {
                    v == "heartbeat" ->
                        klaxon.fromJsonObject(jv.obj!!, Heartbeat::class.java, Heartbeat::class)
                    v == "record" ->
                        klaxon.fromJsonObject(jv.obj!!, Record::class.java, Record::class)
                    else ->
                        throw KlaxonException("value with unexpected value in field \"type\": $jv")
                } } finally { enabled = true }
            }

            override fun toJson(value: Any): String = try { enabled = false; when (value as RecordsMessage) {
                is Heartbeat ->
                        "{\"type\":\"heartbeat\"}"
                is Record ->
                        klaxon.toJsonString(value as Record).dropLast(1) + ",\"type\":\"record\"}"
           } } finally { enabled = true }
        }
    }
...
fun RecordsMessage.Companion.setUpConverters(klaxon: Klaxon) {
    klaxon.converter(RecordsMessage.Record.Record.converter(klaxon))
    klaxon.converter(RecordsMessage.converter(klaxon))
}
